'use strict';
/*
  Import modules/files you may need to correctly run the script.
  Make sure to save your DB's uri in the config file, then import it with a require statement!
 */
var fs = require('fs'),
    mongoose = require('mongoose'),
    Schema = mongoose.Schema,
    Listing = require('./ListingSchema.js'),
    config = require('./config');

/* Connect to your database */
mongoose.connect(config.db.uri);
/*
 Instantiate a mongoose model for each listing object in the JSON file,
  and then save it to your Mongo database
readFile
parse info and save
*/
fs.readFile('C:/Users/david/Documents/GitHub/IntroToSoftware/Assignment3/listings.json', function (err, data) {
  if (err) throw err;
  //console.log(data);
  var listingData = JSON.parse(data);
  console.log(listingData.entries[1].name)

  for(var i=0;i<listingData.entries.length; i++){
    //console.log(listingData.entries[1].name)
    var listing= new Listing({
      name: listingData.entries[i].name,
      code: listingData.entries[i].code,
      coordinates: listingData.entries[i].coordinates,
      address: listingData.entries[i].address



  });
    console.log("Outside save document");
  listing.save(function(err){
    if(err)throw err;
    console.log("save document");
});
}
});
/*
  Once you've written + run the script, check out your MongoLab database to ensure that
  it saved everything correctly.
 */
